#!/usr/bin/env python3
"""
Governance as Engineering — Paper XXVI Consolidated Simulator
==============================================================

THE ESCAPE LADDER AND THE HAZARD OF RETURN

This consolidated simulator reproduces the final-confirmation results for
Paper XXVI. It adds no new institutional mechanisms. Earlier development
cycles remain separate audit-trail scripts.
...
Let K = ceil(N/2). The system reaches half shared adoption or below iff:

    vartheta(theta) > M_N

where:

    vartheta(theta) = -theta - b^2 - L1 - DeltaC
...
No-fitted-boundary phase criterion
------------------------------
...
No phase boundary is fitted to the Monte Carlo map. The predicted phase is
simply whether the finite-N quantile ladder has k*>1.
...
large h. Their spacings can again fall below the linear ladder credit. In the
continuum limit this return to k*=1 requires sigma_0^2/c_*^2 < Lambda/2,
which holds at the frozen parameters.
...
P1 PHASE REPRODUCTION
   The no-fitted-boundary quantile-ladder classification agrees with the empirical
...
P4 SWEEP COMPOSITION
   The median of population-specific hazard-composed exits across dwell values
   has mean absolute error <= 0.012 in theta on new population draws.
...
Run
---
    python3 gae-simulator-v26-cost-of-returning.py

Outputs
-------
    outputs/paper_xxvi-final-phase-map.png
    outputs/paper_xxvi-final-binding-rank-map.png
    outputs/paper_xxvi-final-hazard-curves.png
    outputs/paper_xxvi-final-hazard-validation.png
    outputs/paper_xxvi-final-sweep-composition.png
    outputs/paper_xxvi-final-results.md
"""
...
def run_phase_map() -> dict[str, np.ndarray | float | bool]:
    ...
    misclassified_cells = []
    for n_index, h_index in np.argwhere(mismatch_mask):
        misclassified_cells.append(
            {
                "N": int(MAP_N_VALUES[n_index]),
                "h": float(MAP_H_VALUES[h_index]),
                "empirical_probability": float(
                    empirical_probability[n_index, h_index]
                ),
                "empirical_phase": bool(
                    empirical_majority_phase[n_index, h_index]
                ),
                "predicted_phase": bool(
                    predicted_ladder_phase[n_index, h_index]
                ),
                "predicted_binding_rank": int(
                    predicted_binding_rank[n_index, h_index]
                ),
            }
        )
...
def run_sweep_composition(
    ...
    # New-population generalization with frailty-aware aggregation:
    # compose each training population's own hazard curve first, then aggregate
    # the resulting exit predictions. The old prediction from the median hazard
    # curve is retained only as an audit line in the figure.
    population_prediction_matrix = np.zeros(
        (
            HAZARD_POPULATIONS,
            len(DWELL_VALUES),
        )
    ...
    generalized_prediction_q25 = np.zeros(
        len(DWELL_VALUES)
    )
    generalized_prediction_q75 = np.zeros(
        len(DWELL_VALUES)
    )
    naive_median_hazard_prediction = np.zeros(
        len(DWELL_VALUES)
    )
    ...
    for dwell_index, dwell in enumerate(DWELL_VALUES):
        for population_index in range(HAZARD_POPULATIONS):
            population_prediction_matrix[
                population_index,
                dwell_index,
            ] = predict_exit_from_hazard(
                HAZARD_THETA_VALUES,
        ...
        population_predictions = population_prediction_matrix[
            :,
            dwell_index,
        ]
        generalized_prediction[
            ...
            np.nanmedian(population_predictions)
        )
        generalized_prediction_q25[
            dwell_index
        ] = float(
            np.nanquantile(
        ...
        naive_median_hazard_prediction[
            dwell_index
        ] = predict_exit_from_hazard(
            HAZARD_THETA_VALUES,
            median_hazard_curve,
    ...
    return {
        ...
        "population_prediction_matrix": population_prediction_matrix,
        "generalized_prediction": generalized_prediction,
        "generalized_prediction_q25": generalized_prediction_q25,
        "generalized_prediction_q75": generalized_prediction_q75,
        "naive_median_hazard_prediction": naive_median_hazard_prediction,
...
def save_results(
    phase: dict[str, np.ndarray | float | bool | list],
    ...
    ladder = phase["ladder"]
    high_h = phase["high_h"]
    misclassified = phase["misclassified_cells"]

    median_pointwise_error = np.median(
        np.asarray(hazard["heldout_survival_mae"]),
        axis=0,
    )
    peak_index = int(np.argmax(median_pointwise_error))
    peak_theta = float(HAZARD_THETA_VALUES[peak_index])
    peak_error = float(median_pointwise_error[peak_index])
    ...
    lines = [
        ...
        "",
        f"- No-fitted-boundary classification: "
        f"{int(round(float(phase['classification_accuracy']) * len(MAP_N_VALUES) * len(MAP_H_VALUES)))}"
        f"/{len(MAP_N_VALUES) * len(MAP_H_VALUES)} "
        f"({float(phase['classification_accuracy']):.3f})",
    ...
    if misclassified:
        lines.extend(["", "Misclassified cells:"])
        for cell in misclassified:
            lines.append(
                f"- N={cell['N']}, h={cell['h']:.2f}: "
                f"P(k*>1)={cell['empirical_probability']:.3f}, "
                f"predicted ladder={cell['predicted_phase']}"
            )
    ...
    lines.extend(
        [
            ...
            "## Hazard validation",
            "",
            f"- Overall held-out exponential survival MAE: "
            f"{float(hazard['survival_mae']):.4f}",
            f"- Peak median pointwise MAE: {peak_error:.3f} "
            f"at theta={peak_theta:.2f}",
            "",
            "## Sweep composition",
            "",
            f"- Population-conditional theta MAE: "
            f"{float(sweep['conditional_mae']):.4f}",
            f"- New-population median theta MAE: "
            f"{float(sweep['generalized_mae']):.4f}",
            "",
            "| Dwell | Frailty-corrected median | Prediction IQR | "
            "Median-hazard audit | Observed median |",
            "|---:|---:|---:|---:|---:|",
        ]
    )

    for dwell, predicted, q25, q75, naive, observed in zip(
        DWELL_VALUES,
        sweep["generalized_prediction"],
        sweep["generalized_prediction_q25"],
        sweep["generalized_prediction_q75"],
        sweep["naive_median_hazard_prediction"],
        sweep["generalized_observed"],
    ):
        lines.append(
            f"| {int(dwell)} | {predicted:.3f} | "
            f"[{q25:.3f}, {q75:.3f}] | {naive:.3f} | {observed:.3f} |"
        )
    ...
    json_payload = {
        ...
        "parameters": {
            "N_hazard": HAZARD_N,
            "h_hazard": HAZARD_H,
            "common_shared_bias": COMMON_SHARED_BIAS,
            "independent_error_std": INDEPENDENT_ERROR_STD,
            "liability_ratchet": LIABILITY_RATCHET,
            "passive_retention": PASSIVE_RETENTION,
            "competence_decay": COMPETENCE_DECAY,
            "c_star": C_STAR,
            "mutation_rate": MUTATION_RATE,
            "dwell_values": DWELL_VALUES.tolist(),
        },
        "phase": {
            "classification_accuracy": float(
                phase["classification_accuracy"]
            ),
            "misclassified_cells": misclassified,
            "original": {
                "probability": float(original[0]),
                "predicted_ladder": bool(original[1]),
                "predicted_binding_rank": int(original[2]),
            },
            "ladder": {
                "probability": float(ladder[0]),
                "predicted_ladder": bool(ladder[1]),
                "predicted_binding_rank": int(ladder[2]),
            },
            "high_h": {
                "probability": float(high_h[0]),
                "predicted_ladder": bool(high_h[1]),
                "predicted_binding_rank": int(high_h[2]),
            },
        },
        "hazard": {
            "overall_survival_mae": float(hazard["survival_mae"]),
            "peak_median_pointwise_mae": peak_error,
            "peak_theta": peak_theta,
        },
        "sweep": {
            "conditional_mae": float(sweep["conditional_mae"]),
            "new_population_median_mae": float(
                sweep["generalized_mae"]
            ),
            "frailty_corrected_prediction": np.asarray(
                sweep["generalized_prediction"]
            ).tolist(),
            "prediction_q25": np.asarray(
                sweep["generalized_prediction_q25"]
            ).tolist(),
            "prediction_q75": np.asarray(
                sweep["generalized_prediction_q75"]
            ).tolist(),
            "median_hazard_audit": np.asarray(
                sweep["naive_median_hazard_prediction"]
            ).tolist(),
            "observed": np.asarray(
                sweep["generalized_observed"]
        ...
        "registered_checks": {
            "P1_phase_reproduction": bool(phase["p1"]),
            "P2_anchor_regimes": bool(phase["p2"]),
            "P3_hazard_validation": bool(hazard["p3"]),
            "P4_sweep_composition": bool(sweep["p4"]),
            "P5_dwell_ordering": bool(sweep["p5"]),
        },
    }
    (
        OUTPUT_DIR
        / "paper_xxvi-final-results.json"
    ).write_text(
...
def coordinate_edges(
    ...
    """
    Convert nonuniform cell centers to plotting edges.

    For logarithmic axes, geometric midpoints preserve the spacing implied by
    the displayed scale. For linear axes, arithmetic midpoints are used.
    """
    ...
    if logarithmic:
        if np.any(values <= 0.0):
            raise ValueError("logarithmic coordinate edges require positive values")
...
def main() -> None:
    ...
    print("=" * 100)
    print("PAPER XXVI CONSOLIDATED FINAL CONFIRMATION")
    ...
    print("NO-FITTED-BOUNDARY PHASE CLASSIFICATION")
    total_phase_cells = len(MAP_N_VALUES) * len(MAP_H_VALUES)
    correct_phase_cells = int(
        round(
            float(phase["classification_accuracy"])
            * total_phase_cells
        )
    )
    print(
        f"classification accuracy               : "
        f"{correct_phase_cells}/{total_phase_cells} "
        f"({phase['classification_accuracy']:.3f})"
    )
    for cell in phase["misclassified_cells"]:
        print(
            f"misclassified boundary cell           : "
            f"N={cell['N']}, h={cell['h']:.2f}, "
            f"P(k*>1)={cell['empirical_probability']:.3f}, "
            f"predicted={cell['predicted_phase']}"
        )
    high_h_left = INDEPENDENT_ERROR_STD**2 / C_STAR**2
    high_h_right = (
        2.0 * COMMON_SHARED_BIAS**2
        + LIABILITY_RATCHET
    ) / 2.0
    print(
        f"high-h return condition               : "
        f"{high_h_left:.3f} < {high_h_right:.3f}"
    )
    print(
    ...
    peak_error_index = int(
        np.argmax(median_pointwise_error)
    )
    print(
        f"overall held-out survival MAE         : "
        f"{hazard['survival_mae']:.4f}"
    )
    print(
        f"peak median pointwise MAE             : "
        f"{median_pointwise_error[peak_error_index]:.3f} "
        f"at theta={HAZARD_THETA_VALUES[peak_error_index]:.2f}"
    )
    print(
        f"{'theta':>9}"
        f"{'median lambda':>16}"
    ...
    # Phase probability with the no-fitted-boundary classifier overlaid.
    # The grid is nonuniform in both h and N, so use true cell edges rather
    # than imshow's equally spaced pixels. N is displayed logarithmically.
    ...
    h_edges = coordinate_edges(
        MAP_H_VALUES,
        logarithmic=False,
    )
    n_edges = coordinate_edges(
        MAP_N_VALUES,
        logarithmic=True,
    )
    ...
    if phase["misclassified_cells"]:
        ax.scatter(
            ...
            marker="x",
            s=90,
            label="misclassified boundary cell",
        )
        ax.legend()
    ax.set_yscale("log")
    ax.set_yticks(MAP_N_VALUES)
    ax.set_yticklabels(
    ...
    ax.set_title(
        "Empirical ladder phase with no-fitted-boundary classifier"
    )
    colorbar = fig.colorbar(image, ax=ax)
    colorbar.set_label("P(binding rank k*>1)")
    fig.tight_layout()
    fig.savefig(
        OUTPUT_DIR / "paper_xxvi-final-phase-map.png",
        dpi=170,
    )
    plt.close(fig)

    # Median binding rank map on the same true nonuniform grid.
    fig, ax = plt.subplots(figsize=(9.4, 5.9))
    rank_image = ax.pcolormesh(
        h_edges,
        n_edges,
    ...
    fig.tight_layout()
    fig.savefig(
        OUTPUT_DIR / "paper_xxvi-final-binding-rank-map.png",
        dpi=170,
    )
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(
        OUTPUT_DIR / "paper_xxvi-final-hazard-curves.png",
        dpi=170,
    )
    plt.close(fig)
    ...
    ax.text(
        ...
        0.96,
        (
            f"overall MAE={hazard['survival_mae']:.4f} "
            f"< {MAX_SURVIVAL_MAE:.2f} registered"
        ),
    ...
    ax.set_title(
        "Pointwise degradation toward the shallow, multi-stage region"
    )
    ax.legend()
    fig.tight_layout()
    fig.savefig(
        OUTPUT_DIR / "paper_xxvi-final-hazard-validation.png",
        dpi=170,
    )
    plt.close(fig)

    # Sweep composition with population frailty shown explicitly.
    ...
    ax.fill_between(
        ...
        sweep["generalized_prediction_q75"],
        alpha=0.25,
        label="population-specific prediction IQR",
    )
    ax.plot(
        DWELL_VALUES,
        sweep["generalized_prediction"],
        marker="o",
        label="median of population-specific predictions",
    )
    ax.plot(
        DWELL_VALUES,
        sweep["naive_median_hazard_prediction"],
        marker="x",
        linestyle=":",
        label="prediction from median hazard (audit)",
    )
    ax.plot(
        DWELL_VALUES,
        sweep["generalized_observed"],
    ...
    ax.set_title(
        "Frailty-aware composition: compose first, then aggregate"
    )
    ax.legend()
    fig.tight_layout()
    fig.savefig(
        OUTPUT_DIR / "paper_xxvi-final-sweep-composition.png",
        dpi=170,
    )
    plt.close(fig)
    ...
    print("Saved:")
    for filename in (
        "paper_xxvi-final-phase-map.png",
        "paper_xxvi-final-binding-rank-map.png",
        "paper_xxvi-final-hazard-curves.png",
        "paper_xxvi-final-hazard-validation.png",
        "paper_xxvi-final-sweep-composition.png",
        "paper_xxvi-final-results.md",
        "paper_xxvi-final-results.json",
    ):
